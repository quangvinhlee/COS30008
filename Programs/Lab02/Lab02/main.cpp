...
int main(int argc, char* argv[])
{
	...
	{
		std::cout << "Vertex #" << i << ": " << lPolygon.getVertex(i) <<
			std::endl;
	}
	std::cout << "The perimeter of lPolygon is " << lPolygon.getPerimeter() <<
		std::endl;
	std::cout << "Scale polygon by 3.2:" << std::endl;
	Polygon lScaled = lPolygon.scale(3.2f);
	std::cout << "The perimeter of lScaled is " << lScaled.getPerimeter() <<
		std::endl;
	float lFactor = std::roundf(lScaled.getPerimeter() * 100.0f /
		lPolygon.getPerimeter()) / 100.0f;
