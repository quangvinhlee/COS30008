...
void lambdaIndexer(const DataWrapper& aWrapper)
{
	...
	auto lOrderedMapper = [&aWrapper](size_t aIndex) -> char
		{
			size_t count = 0; // Counter for payload indices
			for (size_t i = 0; i < aWrapper.size(); ++i)
			{
				// Linear search to find the corresponding payload index
				if (aIndex == count)
				{
					// Return the character corresponding to the unsigned value
					return static_cast<char>(aWrapper[i]);
				}
				// Increment the counter based on the payload index
				count += aWrapper[i].size();
			}
			// Return a default character if the index is out of range
